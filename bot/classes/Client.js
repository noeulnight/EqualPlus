...
class eClient extends Client {
  constructor () {
    ...
    this.tt.commandsPath = path() + '/bot/commands'
  ...
  start (token = this.settings.token) {
    this.db = knex({ client: 'mysql', connection: { user: 'equalplus', host: 'localhost', database: 'equalplus' } })
    this.login(token)
  }
